...
class _ReportsChartsState extends State<ReportsCharts> {
  ...
  final CollaboratorController _collabController =
  ...
  Future<void> _loadData() async {
    ...
    final userIds = _events
        .map((e) => e.responsibleId)
        .whereType<String>()
        .toSet();
    final collabIds = <String>{}
      ..addAll(
        _events.map((e) => e.collaboratorCheckedInId).whereType<String>(),
      )
      ..addAll(
        _events.map((e) => e.collaboratorCheckedOutId).whereType<String>(),
      );

    // fetch children and users (they update their caches internally)
    final childFs = childIds
        .map((id) => _childController.fetchChildById(id))
        .toList();
    final userFs = userIds
        .map((id) => _userController.fetchUserById(id))
        .toList();
    // fetch collaborators and insert into collaborators list if fetched
    final collabFs = collabIds
        .map((id) => _collabController.getCollaboratorById(id))
        .toList();
  ...
  List<Attendance> _filterByRange(
    List<Attendance> all,
    DateTime? from,
    DateTime? to,
  ) {
  ...
  Widget build(BuildContext context) {
    if (_attendanceController.isLoadingEvents)
    ...
    final attendanceTiles = events.map((a) {
      final childName =
          _childController.getChildById(a.childId)?.name ?? a.childId ?? '-';
      final responsibleName = a.responsibleId != null
          ? (_userController.getUserById(a.responsibleId!)?.name ??
                a.responsibleId)
          : '-';
      final checkedInBy =
          _collabName(a.collaboratorCheckedInId) ??
          a.collaboratorCheckedInId ??
          '-';
      final checkedOutBy =
          _collabName(a.collaboratorCheckedOutId) ??
          a.collaboratorCheckedOutId ??
          '-';
    ...
    for (final a in events) {
      ...
      if (outId != null)
        collabCheckouts[outId] = (collabCheckouts[outId] ?? 0) + 1;
    }
    ...
    // 5. Checkins by hour
    final Map<int, int> hourly = Map.fromIterable(
    ...
    if (widget.selectedType == 'Check-ins (7d)' ||
        widget.selectedType == 'Check-ins por período') {
      ...
      contentWidgets.addAll([
        const SizedBox(height: 8),
        const Text(
          'Colaboradores (checkins / checkouts)',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 6),
        ...collabCheckins.entries.map(
          (e) => ListTile(
            title: Text(_collabName(e.key) ?? e.key),
            subtitle: Text(
              'Checkins: ${e.value}  •  Checkouts: ${collabCheckouts[e.key] ?? 0}',
            ),
          ),
        ),
      ]);
      ...
      contentWidgets.addAll([
        ...
        const SizedBox(height: 6),
        Text(
          'Total de atendimentos: $total  •  Crianças únicas: $uniqueChildren',
        ),
        const SizedBox(height: 12),
        const Text(
          'Tempo de permanência (min / max / média)',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 6),
        Text(
          'Mínimo: ${minD == null ? '-' : _formatDuration(minD)}  •  Máximo: ${maxD == null ? '-' : _formatDuration(maxD)}  •  Média: ${avgD == null ? '-' : _formatDuration(avgD)}',
        ),
        const SizedBox(height: 12),
        SizedBox(
          height: 180,
          child: _DurationLineChart(
            durationsByDate: _aggregateDurationsByDate(
              events,
              widget.from,
              widget.to,
            ),
      ...
      contentWidgets.addAll([
        const SizedBox(height: 8),
        const Text(
          'Colaboradores ativos (com atividade)',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 6),
        ...collabCheckins.entries
            .where((e) => e.value > 0)
            .map(
              (e) => ListTile(
                title: Text(_collabName(e.key) ?? e.key),
  ...
  String _fmt(DateTime? d) =>
      d == null ? '-' : DateFormat('dd/MM HH:mm').format(d);
  ...
  Map<DateTime, double> _aggregateDurationsByDate(
    ...
    for (final a in events) {
      ...
      final key = DateFormat(
        'yyyy-MM-dd',
      ).format(DateTime(c.year, c.month, c.day));
      final dur = _durationFor(a);
      if (dur != null) {
...
class _DurationLineChart extends StatelessWidget {
  ...
  const _DurationLineChart({Key? key, required this.durationsByDate})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < sorted.length; i++) {
      spots.add(
        FlSpot(i.toDouble(), durationsByDate[sorted[i]]! / 60.0),
      ); // minutes
    }
    return Padding(
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  return Text(
                    DateFormat('dd/MM').format(sorted[idx]),
...
class _HourlyLineChart extends StatelessWidget {
  ...
  const _HourlyLineChart({Key? key, required this.hourlyCounts})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final spots = keys
        .map((k) => FlSpot(k.toDouble(), (hourlyCounts[k] ?? 0).toDouble()))
        .toList();
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 8.0),
      child: LineChart(
